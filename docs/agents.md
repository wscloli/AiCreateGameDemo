...
### ✅ 已完成
- [x] 创建 Player 和敌人的 Sprite 预制体 + 运行时模板节点注册
- [x] BulletFactory 对象池 Prefab 注册
- [x] VFX 特效系统（爆炸、雷击、感电链、受击、引力黑洞、玩家受击、敌人扇形攻击）
- [x] EnvironmentManager（油洼、积水区）
- [x] 肉鸽修改器奖励系统（波次结束三选一，含元素解锁）
- [x] UI 系统（波次 HUD、计时器、GameOver / 胜利面板、奖励选择面板）
- [x] 敌人 AI 多样化（Grunt / Charger / Flanker / Tank）
- [x] 敌人攻击系统（扇形预警 + 0.35s 延迟伤害，美术替换友好）
- [x] 玩家与敌人头顶 HP 条

### ⏳ 待办
1. 场景运行首测 — 在 Cocos Creator 中打开 MainScene.scene，点击预览，观察控制台输出
2. 实现 GLUE 元素及 MONSTER_MERGE 反应（怪物融合）
3. 子弹 Sprite 预制体与发射特效
4. 敌人死亡 / 受击动画资源接入
5. 音效系统（发射、命中、反应、BGM）
